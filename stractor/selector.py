...
class SelectorV1:
    def __init__(self, selector):
        ...
        if self.children is not None:
            self.children = [SelectorV1(x) for x in self.children]
...
class SelectorV1D5:
    def __init__(self, selector):
        ...
        if self.children is not None:
            ...
            tmp_selectors = [SelectorV1D5(x) for x in self.children]
...
#         self.children = selector.get('children')
#         if self.children is not None:
#             self.children = [SelectorV1D5(x) for x in self.children]
...
#         value = []
#         if self.children:
#             # current level has children, so this is not a value node.
#             if self.remove:
#                 for doc in docs:
#                     for remove in self.remove:
#                         rem_ret = remove.query(doc, **kwargs)
...
#                 children_result.append(child.select(selected))
#         else:
#             # current level has no child, so this is a value node.
#             for doc in docs:
#                 item_result = {}
#                 for include in self.include:
#                     val_ret = include.query(doc, **kwargs)
#                     bucket = item_result.setdefault(include.name, [])
#                     bucket += val_ret
#                 if item_result:
#                     value.append(item_result)
